            ...
            <div class="flex flex-col justify-start items-start bg-gray-50 dark:bg-gray-800 w-full p-6 md:p-14">
                ...
                ${cart.map((item) => `
                    ...
                    <div class="flex justify-between w-full items-center">
                        ...
                        <p class="text-lg dark:text-gray-300 font-semibold leading-4 text-gray-600">${item.quantity} x ${new Intl.NumberFormat("de-DE", { style: "currency", currency: "$" }).format(item.price_new)}</p>
                    </div>`).join("")}
                </div>
